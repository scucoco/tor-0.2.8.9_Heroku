...
open(F, "/app/tor-0.2.8.9/src/or/tor --list-torrc-options |") or die;
while (<F>) {
...
loadTorrc("/app/tor-0.2.8.9/src/config/torrc.sample.in", \%torrcSampleOptions);

# Try to figure out what's in the man page.

my $considerNextLine = 0;
open(F, "/app/tor-0.2.8.9/doc/tor.1.txt") or die;
while (<F>) {
